...
''',
      ...
      return rows
          .map((row) => AccountingDocument.fromSql(row, type: type))
...
''',
  ...
  Future<String> createSaleDraft({
    ...
    await _database.transaction((txn) async {
      ...
      for (final item in items) {
        await txn.insert(
          'sale_items',
          item.toSql(entityId: ctx.entityId, saleId: id, now: now),
        );
      }
    });
    return id;
  ...
  Future<void> postSale(String saleId) async {
    ...
    await _database.transaction((txn) async {
      ...
      if (sale['status'] != 'draft')
        throw StateError('Only draft sales can be posted');
      ...
      for (var index = 0; index < items.length; index++) {
        ...
        await _validateUnitInventory(
          txn,
          inventoryItemId,
          item['product_unit_id'] as String,
        );
      ...
      if (partyId == null && paidMinor != finalMinor)
        throw StateError('A credit sale requires a customer');
      if (partyId != null) {
      ...
      if (paidMinor > 0) {
        final cashboxId =
            (sale['cashbox_id'] as String?) ?? ctx.defaultCashboxId;
      ...
      await txn.update(
        'sales',
        {
          ...
          'updated_at': now.toIso8601String(),
          'version': (sale['version'] as num).toInt() + 1,
        },
        where: 'id=?',
        whereArgs: [saleId],
      );
  ...
  Future<String> createPurchaseDraft({
    ...
    await _database.transaction((txn) async {
      ...
      for (final item in items) {
        await txn.insert(
          'purchase_items',
          item.toSql(entityId: ctx.entityId, purchaseId: id, now: now),
        );
      }
    });
    return id;
  ...
  Future<void> postPurchase(String purchaseId) async {
    ...
    await _database.transaction((txn) async {
      ...
      if (purchase['status'] != 'draft')
        throw StateError('Only draft purchases can be posted');
  ...
  Future<String> postWaste({
    required List<WasteLineInput> items,
    String? note,
  }) async {
    ...
    await _database.transaction((txn) async {
      await _ensureYearOpen(txn, ctx.financialYearId);
      var totalCost = 0;
      await txn.insert('waste_invoices', {
        'id': id,
        'entity_id': ctx.entityId,
        'financial_year_id': ctx.financialYearId,
        'waste_number': number,
      ...
      for (final input in items) {
        final inv = await _single(
          txn,
          'inventory_items',
          input.inventoryItemId,
        );
        final available = (inv['current_quantity'] as num).toDouble();
        if (available < input.baseQuantity)
          throw StateError('Insufficient stock for waste');
        final average = await _inventory.currentAverageUnitCostMinor(
          txn,
          input.inventoryItemId,
        );
        ...
        await txn.insert('waste_items', {
          ...
          'unit_factor_at_waste': input.unitFactor,
  ...
  Future<String> postSaleReturn({
    ...
    await _database.transaction((txn) async {
      ...
      if (sale['status'] != 'posted')
        throw StateError('Original sale must be posted');
      ...
      await txn.insert('sale_return_invoices', {
        ...
        'cashbox_id': cashboxId ?? sale['cashbox_id'],
        'note': note,
        'occurred_at': now.toIso8601String(),
        'created_at': now.toIso8601String(),
        'updated_at': now.toIso8601String(),
      ...
      for (final input in items) {
        ...
        if (original['sale_id'] != saleId)
          throw StateError('Return item does not belong to sale');
        ...
        if (cumulativeQty > sold + 0.0000001)
        ...
        final cumulativeAmount = _proportionalByQuantity(
          originalNet,
          cumulativeQty,
          sold,
        );
        ...
        final cumulativeCost = _proportionalByQuantity(
          originalCost,
          cumulativeQty,
          sold,
        );
        ...
        await txn.insert('sale_return_items', {
          'id': itemId,
          'entity_id': ctx.entityId,
          'sale_return_invoice_id': id,
          'sale_item_id': input.originalItemId,
          'inventory_item_id': original['inventory_item_id'],
          'product_unit_id': original['product_unit_id'],
          'quantity': input.quantity,
          'unit_factor_at_return': input.unitFactor,
          'base_quantity': input.baseQuantity,
      ...
      if (refundedMinor > 0) {
        if (refundedMinor > total)
          throw StateError('Refund exceeds return total');
        ...
        if (partyId != null) {
          await _party.recordPartyEntry(
            ...
            balanceDeltaMinor: refundedMinor,
            referenceType: 'sale_return',
            referenceId: id,
            createdBy: ctx.userId,
            originDeviceId: ctx.deviceId,
  ...
  Future<String> postPurchaseReturn({
    ...
    await _database.transaction((txn) async {
      ...
      if (purchase['status'] != 'posted')
        throw StateError('Original purchase must be posted');
      ...
      await txn.insert('purchase_return_invoices', {
        ...
        'discount_minor': 0,
        'final_minor': 0,
        'refunded_minor': receivedMinor,
        'cashbox_id': cashboxId ?? purchase['cashbox_id'],
        'note': note,
        'occurred_at': now.toIso8601String(),
        'created_at': now.toIso8601String(),
        'updated_at': now.toIso8601String(),
      ...
      for (final input in items) {
        final original = await _single(
          txn,
          'purchase_items',
          input.originalItemId,
        );
        if (original['purchase_invoice_id'] != purchaseId)
          throw StateError('Return item does not belong to purchase');
        ...
        final cumulativeQty = alreadyQty + input.baseQuantity;
        if (cumulativeQty > purchased + 0.0000001)
          throw StateError('Return exceeds purchased quantity');
        final inv = await _single(
          txn,
          'inventory_items',
          original['inventory_item_id'] as String,
        );
        if ((inv['current_quantity'] as num).toDouble() + 0.0000001 <
            input.baseQuantity) {
          throw StateError('Insufficient stock to return');
        }
        var originalCost =
            (original['cost_amount_minor'] as num?)?.toInt() ?? 0;
        if (originalCost == 0 &&
            (original['line_total_minor'] as num).toInt() > 0) {
        ...
        final lineTotal = cumulativeAmount - alreadyAmount;
        if (lineTotal < 0)
          throw StateError('Invalid purchase return allocation');
        total += lineTotal;
        final itemId = uuid.v4();
        await txn.insert('purchase_return_items', {
          'id': itemId,
          'entity_id': ctx.entityId,
          'purchase_return_invoice_id': id,
          'purchase_item_id': input.originalItemId,
          'inventory_item_id': original['inventory_item_id'],
          'product_unit_id': original['product_unit_id'],
          'quantity': input.quantity,
          'unit_factor_at_return': input.unitFactor,
      ...
      if (receivedMinor > 0) {
        if (receivedMinor > total)
          throw StateError('Received amount exceeds return total');
        final refundTxId = await _cash.recordCashTransaction(
          txn,
          entityId: ctx.entityId,
          financialYearId: ctx.financialYearId,
        ...
        if (supplierId != null) {
          await _party.recordPartyEntry(
            ...
            balanceDeltaMinor: -receivedMinor,
            referenceType: 'purchase_return',
            referenceId: id,
            createdBy: ctx.userId,
            originDeviceId: ctx.deviceId,
  ...
  Future<void> voidDocument(
    String type,
    String documentId, {
    String reason = 'إلغاء المستند',
  }) async {
    AppLogger.info('void_document', {'type': type, 'id': documentId});
    if (type != 'sale' && type != 'purchase') {
      throw ArgumentError(
        'The v1 event catalog only supports sale and purchase void events',
      );
    }
    final ctx = await LocalContextService.instance.current;
    final table = _tableForType(type);
    await _database.transaction((txn) async {
      ...
      final movements = await txn.query(
        'inventory_movements',
        where:
            'entity_id=? AND reference_type=? AND reference_id=? AND reversal_of_id IS NULL',
      ...
      await _outbox.enqueueEvent(
        ...
        eventType: type == 'sale' ? 'SaleVoided' : 'PurchaseVoided',
        aggregateVersion: (document['version'] as num).toInt() + 1,
        occurredAt: now,
        payload: await _voidPayload(txn, type, documentId, reason, now),
      );
    });
  }
...
''',
  ...
  Future<Map<String, Object?>> _aggregatePayload(
    ...
    final items = await db.rawQuery(
      '''
SELECT i.*, inv.product_id, inv.warehouse_id
FROM $itemTable i
...
''',
    ...
    String? movementFor(Object? itemId) {
      final matches = inventoryMovements
          .where((row) => row['reference_item_id'] == itemId)
          .toList(growable: false);
      return matches.isEmpty ? null : matches.first['id'] as String;
    }

    final firstItem = items.isEmpty ? null : items.first;
    final firstCash = cashTransactions.isEmpty ? null : cashTransactions.first;
    final firstParty = partyEntries.isEmpty ? null : partyEntries.first;
    if (type == 'waste') {
      return <String, Object?>{
        'documentId': id,
        'documentNumber': header['waste_number'],
        'financialYearId': header['financial_year_id'],
        'warehouseId': firstItem?['warehouse_id'],
        'reason': header['note'] ?? 'تلف',
        ...
        'items': items
            .map(
              (item) => <String, Object?>{
                ...
                'unitFactor': item['unit_factor_at_waste'],
    ...
    final isSale = type == 'sale' || type == 'sale_return';
    final isReturn = type == 'sale_return' || type == 'purchase_return';
    final numberColumn = isReturn ? 'return_number' : 'invoice_number';
    final payload = <String, Object?>{
      'documentId': id,
      'documentNumber': header[numberColumn],
      'financialYearId': header['financial_year_id'],
      'partyId': header['party_id'],
      'warehouseId': firstItem?['warehouse_id'],
      'cashboxId': header['cashbox_id'],
      ...
      'items': items
          .map(
            (item) => <String, Object?>{
              ...
              'inventoryMovementId': movementFor(item['id']),
              if (type == 'sale_return')
                'originalSaleItemId': item['sale_item_id'],
              if (type == 'purchase_return')
                'originalPurchaseItemId': item['purchase_item_id'],
            },
          )
          .toList(growable: false),
      'cashTransactionId': firstCash?['id'],
      'partyLedgerEntryId': firstParty?['id'],
    };
    if (type == 'purchase') {
      payload['supplierInvoiceNumber'] = header['supplier_invoice_number'];
    } else if (type == 'sale_return') {
      payload['originalSaleId'] = header['sale_id'];
      payload['refundedMinor'] = header['refunded_minor'];
    } else if (type == 'purchase_return') {
      payload['originalPurchaseId'] = header['purchase_invoice_id'];
      payload['refundedMinor'] = header['refunded_minor'];
    }
    return payload;
  }

  Future<void> _enqueueDocumentEvent(
    DatabaseExecutor db,
    String entityId,
    String type,
    String id,
    ...
    await _outbox.enqueueEvent(
      ...
      payload: await _aggregatePayload(db, type: type, id: id),
    );
  }
  ...
  Future<Map<String, Object?>> _voidPayload(
    ...
    if (reason.trim().isEmpty) throw ArgumentError('Void reason is required');
    final reversals = <Map<String, Object?>>[];
    for (final spec in const [
      ('transactions', 'CASH'),
      ('inventory_movements', 'INVENTORY'),
      ('party_ledger_entries', 'PARTY'),
    ]) {
      final rows = await db.query(
        spec.$1,
        columns: ['id', 'reversal_of_id'],
        where:
            'reference_type=? AND reference_id=? AND reversal_of_id IS NOT NULL',
        whereArgs: ['${type}_void', id],
      );
      reversals.addAll(
        rows.map(
          (row) => {
  ...
  Future<void> _ensureYearOpen(DatabaseExecutor db, String yearId) async {
    ...
    if ((row['is_open'] as num).toInt() != 1)
      throw StateError('Financial year is closed');
  }

  Future<void> _validateUnitInventory(
    DatabaseExecutor db,
    String inventoryItemId,
    String unitId,
  ) async {
    final rows = await db.rawQuery(
      '''SELECT i.product_id inventory_product, u.product_id unit_product FROM inventory_items i JOIN product_units u ON u.id=? WHERE i.id=?''',
      [unitId, inventoryItemId],
    );
    if (rows.isEmpty ||
        rows.first['inventory_product'] != rows.first['unit_product'])
      throw StateError('Product unit does not match inventory item');
  }
