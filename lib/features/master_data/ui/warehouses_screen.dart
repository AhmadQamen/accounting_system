          ...
          children: [
            AnimatedEntrance(
              child: PageIntro(
                ...
                title: 'المستودعات',
                subtitle:
                    'نقاط تخزين واضحة ومستقلة، مع حماية الأرصدة من الحذف غير المقصود.',
                    ...
                    child: Column(
                      ...
                      children: [
                        SectionHeader(
                          ...
                          subtitle:
                              'يمكن تعديل الاسم أو أرشفة المستودع الفارغ فقط',
                          trailing: StatusPill(
                            label: '${rows.length} مستودع',
                        ...
                        if (rows.isEmpty)
                          const EmptyState(
                            ...
                            subtitle: 'أنشئ مستودعاً لتبدأ إدارة المخزون.',
                            icon: Iconsax.buildings_2,
                          )
                        else
                          ...rows.indexed.map(
  ...
  Future<void> _edit(
    ...
    final ok = await showDialog<bool>(
      ...
      builder:
          (dialogContext) => AlertDialog(
            title: Row(
              children: [
                Icon(Iconsax.buildings_2, color: context.colors.primary),
                const SizedBox(width: 10),
                Text(row == null ? 'مستودع جديد' : 'تعديل المستودع'),
              ],
            ),
            content: SizedBox(
    ...
    if (ok == true && controller.text.trim().isNotEmpty) {
      await ref
          ...
          .saveWarehouse(id: row?.id, name: controller.text);
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (dialogContext) => AlertDialog(
            ...
            content: const Text(
              'يمكن أرشفة المستودع فقط إذا لم يكن الافتراضي ولا يحتوي على رصيد مخزون.',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(dialogContext, false),
